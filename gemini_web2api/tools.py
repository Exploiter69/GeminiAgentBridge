...
def _build_tool_choice_instruction(tool_choice, tool_defs: list) -> str:
    ...
    if tool_choice == "required": return "\n\nIMPORTANT: You MUST call at least one tool. Do not respond with text only."
...
def _decode_data_url(url: str):
    ...
    try:
        return (base64.b64decode(match.group(3), validate=True) if match.group(2) else unquote_to_bytes(match.group(3))), mime
...
def _image_from_url(url: str, mime: str = None):
    ...
    return _decode_data_url(url) if url.startswith("data:") else (url, mime or "image/png")
...
def _image_from_part(part: dict):
    ...
    if part_type == "image_url":
        ...
        return _image_from_url(image_url.get("url"), image_url.get("mime_type")) if isinstance(image_url, dict) else _image_from_url(image_url)
    if part_type in ("input_image", "image"):
        image_url = part.get("image_url") or part.get("url")
        if isinstance(image_url, dict): return _image_from_url(image_url.get("url"), image_url.get("mime_type"))
        if image_url: return _image_from_url(image_url, part.get("mime_type"))
        image_data = part.get("data") or part.get("base64")
        if isinstance(image_data, str):
            mime = part.get("mime_type") or part.get("media_type") or "image/png"
            return _decode_data_url(image_data) if image_data.startswith("data:") else _safe_b64_image(image_data, mime)
    return None


def _safe_b64_image(image_data: str, mime: str):
    try: return base64.b64decode(image_data, validate=True), mime
    except (ValueError, TypeError, binascii.Error): return None
    ...
    for msg in messages:
        ...
        if isinstance(content, list):
            ...
            for c in content:
                ...
                if c.get("type") in ("text", "input_text"): text_parts.append(c.get("text", ""))
                else:
                    image = _image_from_part(c)
                    if image: images.append(image); text_parts.append("[Image attached]")
            content = " ".join(text_parts)
        if role == "system": parts.append(f"[System instruction]: {content}")
        elif role == "assistant":
...
def parse_tool_calls(text: str) -> tuple:
    ...
    apply_enum_coercion_in_place(calls, tool_defs)
    errors = validate_tool_choice(tool_choice, tool_defs)
    errors.extend(validate_tool_calls(calls, tool_defs))
    if tool_choice == "none" and calls: errors.append("tool_choice=none forbids tool calls")
    if tool_choice == "required" and not calls: errors.append("tool_choice=required requires at least one tool call")
    if errors:
        clear_tool_context(); raise ValueError("invalid tool call protocol: " + "; ".join(errors[:8]))
    clear_tool_context(); return clean, calls


def build_tool_prompt(tool_defs: list) -> str:
    tool_spec = json.dumps(normalize_tool_definitions(tool_defs), ensure_ascii=False, separators=(",", ":"))
...
def _google_tool_choice_instruction(req: dict) -> str:
    ...
    mode, allowed = fc_config.get("mode", "AUTO"), fc_config.get("allowedFunctionNames", [])
    if mode == "NONE": return "\n\nIMPORTANT: Do NOT call any tools. Respond with text only."
    if mode == "ANY": return "\n\nIMPORTANT: You MUST call " + ("one of these tools: " + ", ".join(f'\"{n}\"' for n in allowed) if allowed else "at least one tool") + ". Do not respond with text only."
    return ""


def google_contents_to_prompt(req: dict) -> tuple:
    parts, images, tool_defs = [], [], []
    fc_mode = req.get("toolConfig", {}).get("functionCallingConfig", {}).get("mode", "AUTO")
    if req.get("tools") and fc_mode != "NONE":
        for group in req["tools"]:
    ...
    elif tool_defs: parts.append(build_tool_prompt(tool_defs) + _google_tool_choice_instruction(req))
    for content in req.get("contents", []):
        role, msg_parts = content.get("role", "user"), []
        for p in content.get("parts", []):
            if p.get("text"): msg_parts.append(p["text"])
            elif p.get("inlineData"):
                data = p["inlineData"]
                try: images.append((base64.b64decode(data["data"], validate=True), data.get("mimeType", "image/png"))); msg_parts.append("[Image attached]")
                except (KeyError, ValueError, TypeError, binascii.Error): pass
            elif p.get("functionCall"):
                fc = p["functionCall"]; msg_parts.append(f'```function_call\n{json.dumps({"name": fc["name"], "args": fc.get("args", {})}, ensure_ascii=False)}\n```')
            elif p.get("functionResponse"):
                fr = p["functionResponse"]; msg_parts.append(f'[Tool result for {fr.get("name", "")}]: {json.dumps(fr.get("response", {}), ensure_ascii=False)}')
        text = "\n".join(msg_parts); parts.append(f"[Assistant]: {text}" if role == "model" else text)
    return "\n\n".join(p for p in parts if p), images
...
def parse_google_function_calls(text: str) -> tuple:
    function_calls = []
    clean = text
    for pattern in [r'```function_call\s*\n(.*?)\n```', r'(?:^|\n)function_call\s*\n(\{.*?\})']:
        for match in re.findall(pattern, clean, re.DOTALL):
            try:
                data = json.loads(match.strip())
                if "name" in data: function_calls.append({"name": data["name"], "args": data.get("args", data.get("arguments", {}))})
            except (json.JSONDecodeError, KeyError): pass
        clean = re.sub(pattern, "", clean, flags=re.DOTALL).strip()
    if not function_calls and clean.strip().startswith("{"):
        try:
            data = json.loads(clean.strip())
            if "name" in data and ("args" in data or "arguments" in data): function_calls.append({"name": data["name"], "args": data.get("args", data.get("arguments", {}))}); clean = ""
        except (json.JSONDecodeError, KeyError): pass
