...
def main() -> int:
    ...
    add("phase_commits_exist_and_are_ancestors", not failures, "all phase commits resolve and are ancestors" if not failures else "; ".join(failures))

    try:
        changed = set(git("diff", "--name-only", PHASE12_BASE, head).splitlines())
        expected = {"scripts/phase13_release_candidate.py", "tests/test_phase13_release_candidate.py", ".github/workflows/phase13-release-candidate.yml", "docs/phase13-release-status.md", "docs/phase8-real-client-evidence.md"}
        add("release_scope_contains_phase13_changes", expected.issubset(changed), "changed paths since Phase 12: " + ", ".join(sorted(changed)))
    ...
    for name, command in [
        ...
        ("roadmap_capability_regression", [sys.executable, "-m", "unittest", "tests.test_roadmap_capabilities", "-v"]),
        ("extension_contract_regression", [sys.executable, "-m", "unittest", "tests.test_extension_contract", "-v"]),
    ...
    if scan.exit_code == 1:
        scan.status = "PASS"; scan.detail = "no credential patterns found"
    ...
    payload = {"phase": 13, "verdict": verdict, "head": head, "branch": branch, "generated_by": "scripts/phase13_release_candidate.py", "agent_claims_used_as_verification": False, "checks": [asdict(c) for c in checks], "failed_checks": [c["name"] for c in failed]}
    ...
    print(json.dumps({"phase": 13, "verdict": verdict, "head": head, "checks": {c.name: c.status for c in checks}, "failed_checks": [c["name"] for c in failed]}, indent=2, sort_keys=True))
    if failed:
